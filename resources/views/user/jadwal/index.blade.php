...
<div class="container mx-auto p-4 max-w-7xl">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col hover:shadow-md transition">
            <h3 class="font-bold text-lg text-gray-800 mb-1">{{ $item->dropOffPoint->name ?? 'Posko SmartWaste' }}</h3>
            <div class="space-y-2 mb-6">
                <p class="text-sm text-gray-500 flex items-center gap-2">
                    <x-mdi-calendar-month-outline class="w-4 text-cyan-500"/> {{ $item->start_date->translatedFormat('d M Y') }}
                </p>
                <p class="text-sm text-gray-500 flex items-center gap-2">
                    <x-mdi-clock-outline class="w-4 text-cyan-500"/> {{ $item->start_date->format('H.i') }} - {{ $item->finish_date->format('H.i') }}
                </p>
                            <div class="mt-4">
        ...
        @empty
        <div class="col-span-full text-center text-sm text-gray-500 py-12">Tidak ada jadwal pengumpulan terdaftar.</div>
        @endforelse
    </div>
</div>
@endsection