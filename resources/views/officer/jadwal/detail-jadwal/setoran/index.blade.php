...
@section('content')
    ...
    <div class="bg-white rounded-[2rem] shadow-xl border border-gray-100 p-6 md:p-8 space-y-8">
        ...
        {{-- ================= GERBANG PROTEKSI STATUS JADWAL ================= --}}
        @if($schedule->status !== 'verified')
            <div class="p-8 bg-amber-50 border-2 border-dashed border-amber-200 rounded-3xl text-center space-y-4">
                ...
                <div class="max-w-md mx-auto space-y-2">
                    <h3 class="text-base font-black text-slate-800 uppercase tracking-wide">Akses Penginputan Timbangan Terkunci</h3>
                    
                    @if($schedule->status === 'rejected')
                        <p class="text-xs font-semibold text-red-600">
                            ❌ Pengajuan harga untuk jadwal ini **DITOLAK** oleh Assessor.
                        </p>
                        @if($schedule->rejection_reason)
                            <div class="p-3 bg-red-50 border border-red-100 rounded-xl text-left text-xs text-red-700 font-medium mt-2">
                                <strong>Catatan Penolakan:</strong> "{{ $schedule->rejection_reason }}"
                            </div>
                        @endif
                        <p class="text-[11px] text-gray-400 pt-1">Silakan masuk ke menu <span class="font-bold text-slate-700">Atur Nilai Tukar / Harga</span> untuk menyesuaikan nominal kembali.</p>
                    @else
                        <p class="text-xs font-semibold text-amber-700">
                            ⏳ Status jadwal saat ini: **Menunggu Review Harga**. Form timbangan terbuka otomatis setelah Assessor menyetujui nominal harga berjalan yang Anda ajukan.
                        </p>
                    @endif
                </div>
            </div>
        @else
            {{-- FORM HANYA MUNCUL JIKA STATUS === 'VERIFIED' --}}
            <form id="deposit-multiform" action="{{ route('officer.jadwal.detail.setoran.store', $schedule->id) }}" method="POST" class="space-y-8">
                @csrf
                @method('POST')
                
                {{-- SECTION 1: DATA INDUK SETORAN --}}
                ...
                {{-- SECTION 2: DINAMIS BARIS SAMPAH --}}
                <div class="space-y-4">
    ...
    <script type="module">
        $(document).ready(function() {
            ...
            $('#add-row-btn').on('click', function() {
                ...
                setTimeout(() => { newRow.removeClass('opacity-0 scale-95'); }, 50);
                rowIndex++;
                toggleRemoveButtons();
            });
